...
class SemanticAnalyzer:
    ...
    def generate_keyword_report(self, keyword_results: Dict) -> str:
        """生成关键词分析报告"""
        report = "语义关键词分析报告\n"
        report += "=" * 50 + "\n\n"
        ...
        for label, data in keyword_results.items():
            ...
            if keywords:
                # 语义关键词
                semantic_keywords = keywords.get('semantic_keywords', [])
                if semantic_keywords:
                    report += "语义关键词:\n"
                    for i, (word, score) in enumerate(semantic_keywords[:10], 1):
                        report += f"  {i}. {word} (重要性: {score:.4f})\n"
                else:
                    report += "语义关键词: 无数据\n"
                ...
                if contextual_keywords:
                    report += f"\n上下文关键词:\n"
                    for i, (word, score) in enumerate(contextual_keywords[:10], 1):
                        report += f"  {i}. {word} (重要性: {score:.4f})\n"
                else:
                    report += f"\n上下文关键词: 无数据\n"
                
                # 综合关键词
                combined_keywords = keywords.get('combined_keywords', [])
                if combined_keywords:
                    report += f"\n综合关键词:\n"
                    for i, (word, score) in enumerate(combined_keywords[:10], 1):
                        report += f"  {i}. {word} (综合分数: {score:.4f})\n"
                else:
                    report += f"\n综合关键词: 无数据\n"
            else:
                report += "关键词信息: 无数据\n"
...
if __name__ == "__main__":
    ...
    # 测试关键词提取
    test_texts = df['cleaned_text'].tolist()
    keywords = analyzer.extract_advanced_keywords(test_texts, top_k=10)
    print(f"提取的关键词: {keywords}")
    
    # 测试数据增强分析
    keyword_results = analyzer.analyze_for_augmentation(df)
    print(f"关键词分析完成，分析了 {len(keyword_results)} 个标签")
    
    # 生成报告
    report = analyzer.generate_keyword_report(keyword_results)
    print("关键词分析报告:")
    print(report[:500] + "...")
