...
### 1. DataPreprocessor（优化版）
**功能**: 数据预处理和清理
**技术特点**:
- OCR文本清理和标准化（保留换行符）
- 特殊字符处理
- **统一模式管理**：集中定义所有正则表达式模式，避免重复
- **扩展结构化信息提取**：
  - URL、电话、联系方式（支持国际号码和多种社交平台）
  - 加密货币地址（Bitcoin、Ethereum、Litecoin、Dogecoin等）
  - 银行信息（银行卡号、银行名称、英文缩写）
  - 可疑模式（按类别分组的诈骗关键词）
- 数据质量验证
...
- 上下文相关关键词分析
- 标签特定关键词识别
- **模式复用优化**：复用DataPreprocessor的统一模式，避免重复定义
...
- **缓存机制**: 实现结果缓存，避免重复计算
- **统一模式管理**: 避免重复定义，提升处理效率
- **模式复用**: 关键词提取模块复用数据预处理模块的模式
