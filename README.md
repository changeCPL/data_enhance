...
### 深度学习集成
- **统一语义分析服务**: 集成BERT/SentenceTransformers模型，提供统一的语义分析接口
- **高级关键词提取**: 基于深度学习的语义关键词和上下文关键词提取，支持多维度评分和图论分析
- **综合关键词提取**: 结合语义和上下文关键词，提供更全面的关键词分析
- **智能缓存机制**: 特征缓存和结果缓存，显著提升处理效率
...
- **高级语义关键词提取**：
  - 语义重要性计算（多维度评分：语义差异(60%) + 词频(30%) + 位置权重(10%)）
  - 上下文关键词分析（基于图论的复杂语义关系分析）
  - 综合关键词（语义+上下文加权合并）
  - 智能缓存机制（特征缓存、结果缓存）
- **标签特定关键词识别**：网站名、动词提取
- **性能优化**：
...
- 可视化功能（PCA降维、语义空间可视化）
- **高级关键词提取**：
  - 语义关键词提取（多维度评分：语义重要性60% + 词频30% + 位置权重10%）
  - 上下文关键词提取（图论方法、聚类预筛选）
  - 综合关键词提取（加权合并算法）
- 智能缓存机制（特征缓存、结果缓存）
